...
export default function FashionBeautyServices() {
  ...
  const services = ['Bridal Makeup', 'Airbrush Makeup', 'Hair Styling', 'Saree Draping', 'Hair Extensions', 'Traditional Dressing', 'Pre-Wedding Trial'];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Results Count */}
        <div className="mb-6">
          <p className="text-gray-600">Showing <span className="font-semibold">{filteredVendors.length}</span> beauty service{filteredVendors.length !== 1 ? 's' : ''}</p>
        </div>

        {/* Vendor Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredVendors.map(vendor => (
            <div key={vendor.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow cursor-pointer" onClick={() => router.push(`/services/fashion-beauty/${vendor.id}`)}>
              ...
              <div className="p-5">
                ...
                <div className="mb-3">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {vendor.services.length > 3 && <span className="px-2 py-1 text-xs text-gray-500">+{vendor.services.length - 3} more</span>}
                  </div>
                </div>
        ...
        {filteredVendors.length === 0 && (
          <div className="text-center py-16">
            <p className="text-xl text-gray-500 mb-4">No beauty services found matching your criteria</p>
